...
def test_parse_csv_impl_format_a_uses_vendor_hint_and_validates_clean():
    ...
    state = {"_file_bytes": data, "_detected_format": "format_a_csv", "_vendor_hint": "Getwell Medicare Solution"}
    result = agent_tools._parse_csv_impl(state)
    assert result["vendor"] == "Getwell Medicare Solution"
...
def test_parse_xls_impl_parses_and_validates_clean():
    ...
    state = {"_file_bytes": f.read_bytes(), "_detected_format": "format_c_xls", "_vendor_hint": "Bruklyn Associates"}
    result = agent_tools._parse_xls_impl(state)
    assert result["vendor"] == "Bruklyn Associates"
    assert result["validation_issues"] == []
