...
def test_find_related_document_returns_none_with_no_candidates():
    assert find_related_document("PH-26-49832", "Getwell Medicare Solution", []) is None
...
def test_find_related_document_skips_unparseable_candidate():
    ...
    result = find_related_document(
        "PH-26-49832", "Getwell Medicare Solution", [("empty.csv", garbage)]
    )
...
def test_find_related_document_finds_authoritative_pdf_for_reconciliation():
    # rptGSTSALESINVOICEGMSPL...pdf is Getwell Medicare's own PDF for
    ...
    pdf = (SAMPLES_DIR / "rptGSTSALESINVOICEGMSPL650172026215756654.pdf").read_bytes()
